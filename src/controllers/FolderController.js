...
class FolderController {
  async createFolder(req, res, next) {
    try {
      ...
      const folder = await folderService.createFolder(userId, parentId, name);
  ...
  async getFolderChildren(req, res, next) {
    try {
      const { id } = req.params;
      const { includeDocuments, page, pageSize } = req.query;
      
      const result = await folderService.getFolderChildren(
        id || null,
        { includeDocuments: includeDocuments !== 'false', page, pageSize }
      );
